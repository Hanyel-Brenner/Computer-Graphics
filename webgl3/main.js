...
*/
   ...
   /* for(let i=0; i<flowerCircles.length; i++)
    ...
    for(let i=0; i<clownCircles.length; i++)
        {
            gl.bindBuffer(gl.ARRAY_BUFFER, positionBuffer);
            setCircleVertices(gl,clownCircles[i][0], clownCircles[i][1], clownCircles[i][2], N_OF_CIRCLE_POINTS)
            gl.bindBuffer(gl.ARRAY_BUFFER, colorBuffer);
            setCircleColor(gl,clownCirclesColor[i][0],clownCirclesColor[i][1],clownCirclesColor[i][2], N_OF_CIRCLE_POINTS);
    
            gl.drawArrays(gl.TRIANGLES,0, N_OF_CIRCLE_POINTS*6);
        }

    for(let i=0; i<clownTriangles.length; i++)
        {
            gl.bindBuffer(gl.ARRAY_BUFFER, positionBuffer);
            setRectangleVertices(gl,clownTriangles[i][0],clownTriangles[i][1],clownTriangles[i][2],clownTriangles[i][3]);
            gl.bindBuffer(gl.ARRAY_BUFFER, colorBuffer);
            setRectangleColor(gl,clownTrianglesColor[i][0],clownTrianglesColor[i][1],clownTrianglesColor[i][2]);
